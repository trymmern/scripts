param(
    [Parameter(Mandatory = $true)][bool] $CIRun
)

$CIRun = [bool]::Parse($CIRun)
...
# The dependencies of the different test projects !!!NB: The actual ".IntegrationTests" project needs to be at index 0 of each list for it to output correctly in the console.
...
function Main([bool]$CIRun)
{
    ...
    Write-Output "Diffing: $($branch) -> $($targetBranch)"
    $files = $(git diff $targetBranch...$branch --name-only)
    $fileList = $files.Split(' ')
    $projects = $fileList | % { $_.Split('/')[1] } | Get-Unique
    
    Write-Output "`nDetected changes in files ($($fileList.Length)):"
    foreach ($file in $fileList) 
    {
        Write-Output $file
    }

    Write-Output "`nDetected changes in the following projects ($($projects.Length)):"
    foreach ($project in $projects)
    {
        Write-Output $project
    }

    Write-Output "`nChecking projects...`n"
    ...
    if ($contractsChanged -or $commonsChanged -or $efDatabaseChanged -or $shChanged) 
    {
        ...
        if ($contractsChanged) {Write-Output $contractsChanged[0]}
        if ($commonsChanged) {Write-Output $commonsChanged[0]}
        if ($efDatabaseChanged) {Write-Output $efDatabaseChanged[0]}
        if ($shChanged) {Write-Output $shChanged[0]}
        Write-Output "Breaking changes detected in the projects. Running all tests"
        Exit
    }
...
function FilterTestRunners([bool]$CIRun, [string]$Project)
{
    if ($CIRun)
    {
        SetTestToggle -Project $Project -Deps $global:ImportersDeps -SkipTestToggle $(SkipImportersTests)
        SetTestToggle -Project $Project -Deps $global:ProxyServerDeps -SkipTestToggle $(SkipProxyServerTests)
        SetTestToggle -Project $Project -Deps $global:ArDeps -SkipTestToggle $(SkipArTests)
        SetTestToggle -Project $Project -Deps $global:ArProxyDeps -SkipTestToggle $(SkipArProxyTests)
        SetTestToggle -Project $Project -Deps $global:BusinessDeps -SkipTestToggle $(SkipBusinessTests)
        SetTestToggle -Project $Project -Deps $global:CodesDeps -SkipTestToggle $(SkipCodesTests)
        SetTestToggle -Project $Project -Deps $global:CommonDeps -SkipTestToggle $(SkipCommonTests)
        SetTestToggle -Project $Project -Deps $global:CppaDeps -SkipTestToggle $(SkipCppaTests)
        SetTestToggle -Project $Project -Deps $global:FlrDeps -SkipTestToggle $(SkipFlrTests)
        SetTestToggle -Project $Project -Deps $global:HprDeps -SkipTestToggle $(SkipHprTests)
        SetTestToggle -Project $Project -Deps $global:HtkDeps -SkipTestToggle $(SkipHtkTests)
        SetTestToggle -Project $Project -Deps $global:KrkDeps -SkipTestToggle $(SkipKrkTests)
        SetTestToggle -Project $Project -Deps $global:LsrDeps -SkipTestToggle $(SkipLsrTests)
        SetTestToggle -Project $Project -Deps $global:OfrDeps -SkipTestToggle $(SkipOfrTests)
        SetTestToggle -Project $Project -Deps $global:PtDeps -SkipTestToggle $(SkipPtTests)
        SetTestToggle -Project $Project -Deps $global:RabbitMqMigrationDeps -SkipTestToggle $(SkipRabbitMqMigrationTests)
        SetTestToggle -Project $Project -Deps $global:ReshDeps -SkipTestToggle $(SkipReshTests)
        SetTestToggle -Project $Project -Deps $global:UserAdminDeps -SkipTestToggle $(SkipUserAdminTests)
    }
    else
    {
...
function ToggleAllTests([bool]$CIRun, [bool]$Skip)
{
    if ($CIRun)
    {
        Write-Output "##vso[task.setvariable variable=SkipImportersTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipProxyServerTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipArTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipArProxyTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipBusinessTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipCodesTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipCommonTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipCppaTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipFlrTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipHprTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipHtkTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipKrkTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipLsrTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipOfrTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipPtTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipRabbitMqMigrationTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipReshTests;isOutput=true;]$Skip"
        Write-Output "##vso[task.setvariable variable=SkipUserAdminTests;isOutput=true;]$Skip"
    }
    else
    {
